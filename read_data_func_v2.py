...
#test coincidence counter

test_G5 = [1,2,10,1,0,0]
test_G7 = [1,2,10,0,1,0]

def get_coincidence(filename, mode = '57'):
    # Takes in full data file and the two target ions (channels 5,7,9)
    # Outputs a list of conincidence rates (both up with counts > 1) over the 100 expts per point
        ...
        if mode == '59':
            for i in range(len(G5)):
                if G5[i] > 1 and G9[i] > 1:
                    ...
                    coincidences.append(0)
        result.append(sum(coincidences)/len(coincidences))
    return result
