...
def get_coincidence(filename):
    # Takes in full data file (single-moded) and
    # Outputs a list of coincidence rates (both up with counts > 1) over the 100 expts per point
    # Outputs a tuple of 12 lists for each (spin and target ion) combination
    # UpUp (57 -> 79 -> 59) -> UpDown (57 -> 79 -> 59) -> DownUp (57 -> 79 -> 59) -> DownDown (57 -> 79 -> 59)
    d,h,r,t = read_file(filename)
    output = process_raw(r)
    result = []
    subresult57 = []
    subresult79 = []
    subresult59 = []
    for x in output:
        ...
        n = len(G5)
        for i in range(n):
                if G5[i] > 1 and G7[i] > 1:
                    fst_sec += 1
        subresult57.append(fst_sec/n)
        for i in range(n):
                if G9[i] > 1 and G7[i] > 1:
                    sec_thd += 1
        subresult79.append(sec_thd/n)
        for i in range(n):
                if G5[i] > 1 and G9[i] > 1:
                    fst_thd += 1
        subresult59.append(fst_thd/n)
    result.append(subresult57)
    result.append(subresult79)
    result.append(subresult59)
    ##########################
    subresult57 = []
    subresult79 = []
    subresult59 = []
    for x in output:
        ...
        n = len(G5)
        for i in range(n):
                if G5[i] > 1 and G7[i] < 2:
                    fst_sec += 1
        subresult57.append(fst_sec/n)
        for i in range(n):
                if G9[i] < 2 and G7[i] > 1:
                    sec_thd += 1
        subresult79.append(sec_thd/n)
        for i in range(n):
                if G5[i] > 1 and G9[i] < 2:
                    fst_thd += 1
        subresult59.append(fst_thd/n)
    result.append(subresult57)
    result.append(subresult79)
    ...
    for x in output:
        ...
        n = len(G5)
        for i in range(n):
                if G5[i] < 2 and G7[i] > 1:
                    fst_sec += 1
        subresult57.append(fst_sec/n)
        for i in range(n):
                if G9[i] > 1 and G7[i] < 2:
                    sec_thd += 1
        subresult79.append(sec_thd/n)
        for i in range(n):
                if G5[i] < 2 and G9[i] > 1:
                    fst_thd += 1
        subresult59.append(fst_thd/n)
    result.append(subresult57)
    result.append(subresult79)
    ...
    for x in output:
        ...
        n = len(G5)
        for i in range(n):
                if G5[i] < 2 and G7[i] < 2:
                    fst_sec += 1
        subresult57.append(fst_sec/n)
        for i in range(n):
                if G9[i] < 2 and G7[i] < 2:
                    sec_thd += 1
        subresult79.append(sec_thd/n)
        for i in range(n):
                if G5[i] < 2 and G9[i] < 2:
                    fst_thd += 1
        subresult59.append(fst_thd/n)
    result.append(subresult57)
    result.append(subresult79)
    ...
    for comb in result:
        suberr = []
        for i in comb:
            suberr.append(np.sqrt(i*(1-i)/n))
        err.append(suberr)
    ##########################
    return (d[0],0,0,result,err)
